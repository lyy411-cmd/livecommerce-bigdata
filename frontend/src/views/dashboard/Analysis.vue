<template>
  <div class="analysis">
    ...
    <el-row :gutter="16" class="mt-16">
      ...
      <el-col :span="12">
        <div class="section-card">
          <div class="section-header"><div><h3>各平台主播数与平均GMV</h3><p class="sub">比较各平台主播资源</p></div></div>
    ...
    <div class="section-card mt-16">
      <div class="section-header">
        <div>
          <h3>主播 vs 类目 热力图</h3>
          <p class="sub">颜色越深表示该主播在该类目的带货订单越多</p>
        </div>
      </div>
...
const initPlatformAnchor = () => {
  if (!platformAnchorRef.value || anchors.value.length === 0) return
  const platforms = ['抖音', '淘宝', '快手']
  const counts = anchors.value.reduce((acc, a) => {
    const p = {douyin:'抖音', taobao:'淘宝', kuaishou:'快手'}[a.platform]
    acc[p] = (acc[p] || 0) + 1
    return acc
  }, {})
  const avgGmv = platforms.map(p => {
    const list = anchors.value.filter(a => ({douyin:'抖音', taobao:'淘宝', kuaishou:'快手'}[a.platform]) === p)
    return list.length > 0 ? +(list.reduce((s, a) => s + a.totalGmv, 0) / list.length / 100000000).toFixed(1) : 0
  })
  ...
  c.setOption({
    ...
    grid: { left: 60, right: 60, top: 40, bottom: 30 },
    xAxis: { type: 'category', data: platforms, axisLabel: { color: 'rgba(255,255,255,0.5)' }, axisLine: { lineStyle: { color: 'rgba(255,255,255,0.15)' } } },
    yAxis: [
    ...
    series: [
      { name: '主播数', type: 'bar', data: platforms.map(p => counts[p] || 0),
        itemStyle: { color: '#a855f7', borderRadius: [4, 4, 0, 0], shadowBlur: 6, shadowColor: 'rgba(168,85,247,0.3)' } },
      { name: '平均GMV(亿)', type: 'line', yAxisIndex: 1, data: avgGmv,
...
const initHeat = () => {
  ...
  for (let i = 0; i < platforms.length; i++) {
    for (let j = 0; j < categories.length; j++) {
      const count = anchors.value.filter(a => 
        ({douyin:'抖音', taobao:'淘宝', kuaishou:'快手'}[a.platform]) === platforms[i] && a.category === categories[j]
  ...
  c.setOption({
    ...
    tooltip: { position: 'top', backgroundColor: 'rgba(15,20,30,0.95)', borderColor: 'rgba(0,255,204,0.3)', textStyle: { color: '#e0e0e0' },
      formatter: (p) => `${platforms[p.value[1]]} - ${categories[p.value[0]]}<br/>订单: ${p.value[2]}单` },
    ...
    yAxis: { type: 'category', data: platforms, splitArea: { show: true, areaStyle: { color: ['rgba(255,255,255,0.02)', 'rgba(255,255,255,0.04)'] } }, axisLabel: { color: 'rgba(255,255,255,0.5)' }, axisLine: { lineStyle: { color: 'rgba(255,255,255,0.1)' } } },
    visualMap: {
